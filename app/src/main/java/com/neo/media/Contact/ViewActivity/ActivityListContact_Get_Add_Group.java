...
import com.neo.media.Contact.Adapter.Adapter_Contact_Get_Add_Group;
...
public class ActivityListContact_Get_Add_Group extends BaseActivity {
    public static final int MY_PERMISSIONS_REQUEST_READ_CONTACTS = 123;
    public static String TAG = ActivityListContact_Get_Add_Group.class.getSimpleName();
    public static ActivityListContact_Get_Add_Group fragment;
    private List<PhoneContactModel> lisService;
    private Adapter_Contact_Get_Add_Group adapterService;
    ...
    public static ActivityListContact_Get_Add_Group getInstance() {
        if (fragment == null) {
            synchronized (ActivityListContact_Get_Add_Group.class) {
                if (fragment == null)
                    fragment = new ActivityListContact_Get_Add_Group();
            }
        }
        return fragment;
    }
            ...
            public void onClick(View v) {
                for (int i = 0; i < temp.size(); i++) {
                    if (temp.get(i).isChecked()) {
                        MyApplication.list_ct_add_group.add(temp.get(i));
                    }
                }
                if (MyApplication.list_ct_add_group.size() > 0) {
                    finish();
                } else
                    Toast.makeText(ActivityListContact_Get_Add_Group.this, "Bạn chưa chọn số điện thoại nào", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    void filter(String text) {
        ...
        adapterService.setOnIListener(new setOnItemClickListener() {
            ...
            public void OnItemClickListener(int position) {
                temp.get(position).setChecked(!temp.get(position).isChecked());
                adapterService.updateList(temp);
                int count = 0;
                for (int i = 0; i < temp.size(); i++) {
                    if (temp.get(i).isChecked()) {
                ...
                if (count > 13) {
                    new SweetAlertDialog(ActivityListContact_Get_Add_Group.this, SweetAlertDialog.NORMAL_TYPE)
                            .setTitleText("Thông báo")
                            .setContentText("Bạn chỉ được thêm tối đa là 15 thành viên trong một nhóm")
                            ...
                            .show();
                    // show_notification("Thông báo", "Bạn chỉ được thêm tối đa là 15 thành viên trong một nhóm");
                    //    finish();
                }
            }
    ...
    private void init() {
        adapterService = new Adapter_Contact_Get_Add_Group(temp, this);
    ...
    protected void onResume() {
        super.onResume();
        btn_ok_service.setVisibility(View.VISIBLE);
    }
