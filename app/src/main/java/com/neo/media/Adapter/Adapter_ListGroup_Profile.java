...
public class Adapter_ListGroup_Profile extends RecyclerView.Adapter<Adapter_ListGroup_Profile.TopicViewHoder> {
    private List<GroupName> lisItem;
    ...
    public Adapter_ListGroup_Profile(List<GroupName> items, Context context) {
        this.lisItem = items;
    ...
    public void onBindViewHolder(TopicViewHoder holder, final int position) {
        final GroupName item = lisItem.get(position);
        if (item.getId_group() != null)
            holder.txt_name_time.setText("Nhóm: "+item.getsNameLocal());
        else
            holder.txt_name_time.setText(item.getsNameLocal());
        if (item.isCheck()) {
