...
public class AdapterHistoryKeyWord extends RecyclerView.Adapter<AdapterHistoryKeyWord.TopicViewHoder> {
    private ArrayList<String> lisString;
    ...
    public AdapterHistoryKeyWord(ArrayList<String> items, Context context) {
        this.lisString = items;
    ...
    public void onBindViewHolder(TopicViewHoder holder, final int position) {
        final String item = lisString.get(position);
        holder.txt_name_time.setText(item);
        holder.icon_time_group.setVisibility(View.GONE);
       /* if (item.isCheck()) {
