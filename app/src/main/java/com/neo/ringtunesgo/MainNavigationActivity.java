...
import com.neo.ringtunesgo.Fragment.Stop_Pause.FragmentStopPause;
import com.neo.ringtunesgo.MainActivity.FragmetSearch;
import com.neo.ringtunesgo.MainActivity.MainActivityImpl;
import com.neo.ringtunesgo.MainActivity.PresenterMainActivity;
import com.neo.ringtunesgo.Model.DrawerItem;
...
import com.neo.ringtunesgo.untils.Storage;
...
public class MainNavigationActivity extends BaseActivity
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (!isLogin) {
                    ...
                    if (position == 1) {
                        ...
                        if (!FragmentStopPause.getInstance().isAdded())
                            FragmentUtil.addFragment(MainNavigationActivity.this, FragmentStopPause.getInstance(), true);
                        drawer.closeDrawers();
                    } else if (position == 10) {
    ...
    private void reloadDrawer() {
        ...
        if (is_subscriber && is_SVC_STATUS) {
            drawerIte8.setItemName("Tạm dừng| Huỷ");
        } else drawerIte8.setItemName("Đăng ký");
    ...
    public void addDrawerItem() {
        ...
        lisDrawer.add(drawerItem4);
        DrawerItem drawerItem5 = new DrawerItem();
        drawerItem5.setImgResID(R.drawable.singer_blue);
    ...
    protected void onResume() {
        ...
        Info_User obj = myRealm.where(Info_User.class).findFirst();
        if (objLoginAll != null) {
            if (objLoginAll.isLogin())
                isLogin = objLoginAll.isLogin();
            if (objInfo != null) {
                if (obj.getStatus() != null && obj.getStatus().equals("2")) {
                    is_subscriber = true;
                    if (obj.getService_status().equals("1")) {
                        is_SVC_STATUS = true;
                    }
                } else {
                    is_subscriber = false;
